...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["creare_transfer"])) {
    $ID_cont = $_POST["ID_Cont"];
    $ID_Cont_Destinatie = $_POST["ID_Cont_Destinatie"];
    $Tip_Tranzactie = "Transfer";
    $suma = $_POST["suma"];
    ...
    if ($checkOwnershipResult->num_rows > 0) {
        $stmt = $conn->prepare("INSERT INTO tranzactii (Tip_Tranzactie, Suma, ID_Cont, ID_Cont_Destinatie) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("sdii", $Tip_Tranzactie, $suma, $ID_cont, $ID_Cont_Destinatie);

        if ($stmt->execute()) {
            header("Location: tranzactii.php");
            exit();
        } else {
            $errorMsg = "Error: " . $stmt->error . "<br>Debugging: Tranzaction creation failed.";
        }

        $stmt->close();
    } else {
        $errorMsg = "Nu ai dreptul sa faci transfer din acest cont.";
    }
...
<div class="content-container">
    <div class="form-container">
        <h2>Transfer</h2>
        <form action="creareTransfer.php" method="post">
            <label for="ID_Cont">ID Cont:</label>
            <input type="number" name="ID_Cont" required><br>

            <label for="ID_Cont_Destinatie">ID Cont Destinatie:</label>
            <input type="number" name="ID_Cont_Destinatie" required><br>

            <label for="suma">Suma:</label>
            <input type="number" name="suma" required><br>
            <input type="submit" name="creare_transfer" value="Transfer">
        </form>
